        ...
        <div class="@if(isset($sidebar))col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 @endif main">
          ...
          @if(isset($rows) && count($rows))
            <div class="table-responsive">
                    ...
                    <tr>
                      ...
                      <td>
                          <div class="btn-group btn-group-sm">
                            <a href="{{ URL::to('admin/'.$view.'/'.$one->id.'/edit') }}" class="btn btn-default" data-placement="top" title="Edit">
                              <span class="glyphicon glyphicon-pencil"></span>
                            </a>
                        @if(isset($one->dates) && in_array('deleted_at', $one->dates))
                          @if($one->trashed())
                            <a href="{{ URL::to('admin/'.$view.'/'.$one->id.'/restore') }}" class="btn btn-success" data-placement="top" title="Restore">
                              <span class="glyphicon glyphicon-refresh"></span>
                            </a>
                          @else
                            <a href="{{ URL::to('admin/'.$view.'/'.$one->id.'/trash') }}" class="btn btn-warning" data-placement="top" title="Trash">
                              <span class="glyphicon glyphicon-trash"></span>
                            </a>
                          @endif
                        @endif
                            <a href="#" data-toggle="modal" data-target="#confirm_delete" data-id="{{ $one->id }}" data-action="{{ URL::to('admin/'.$view.'/'.$one->id.'/destroy') }}" class="btn btn-danger" data-placement="top" title="Delete">
              ...
              {{ method_exists($rows, 'links') ? $rows->links('admin.partials.pagination') : '' }}
            </div>
