...
@section('content')
    ...
    <div class="@if(isset($sidebar)) col-sm-offset-3 col-md-offset-2 @endif col-md-10 col-sm-9 main">
        ...
        <h1 class="page-header">{{ trans('reactiveadmin::reactiveadmin.create.title') }} @if($config['singular_title']){{{ '"'.$config['singular_title'].'"' }}}@else{{{ '"'.$model.'"' }}}@endif</h1>

        <!-- Tab panes -->
        {{ Form::open(array('url' => array(Config::get('reactiveadmin::uri').'/'.$view), 'method' => 'post', 'class' => 'form-horizontal', 'role' => 'form', 'enctype' => 'multipart/form-data')) }}

        @include('reactiveadmin::partials.create_ajax')

        <div class="form-group">
            <div class="col-sm-12">
                <button type="submit" class="btn btn-lg btn-primary pull-right">{{ trans('reactiveadmin::reactiveadmin.create.title') }}</button>
            </div>
        </div>
        {{ Form::close() }}
    </div>
